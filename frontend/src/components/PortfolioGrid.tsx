  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => (
        <article
          key={item.id}
          className="overflow-hidden rounded-lg border border-border bg-card hover:shadow-lg transition-shadow"
        >
                  ...
                  <div className="mb-2 flex flex-wrap items-center gap-3 text-xs sm:text-sm text-card-foreground">
                    ...
                    {item.surface ? (
                      <span className="inline-flex items-center gap-1">
                        ...
                        {item.surface}
                        {/\d/.test(item.surface) && !/m/i.test(item.surface) ? " m²" : ""}
                      </span>
                    ) : null}
                  </div>
                )}
                {item.description ? (
                  <p className="mb-2 text-sm text-muted-foreground line-clamp-3">{item.description}</p>
                ) : null}
                {item.price ? (
                  <p className="border-t border-border pt-2 text-base font-bold text-card-foreground">
                    {item.price}
                  </p>
                ) : null}
              </>
